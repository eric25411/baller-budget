...
function getPeriod() {
    ...
    return { startStr: start.toISOString().split('T')[0], endStr: end.toISOString().split('T')[0], label: `${start.toLocaleDateString('en-US', {month:'short', day:'numeric'})} - ${end.toLocaleDateString('en-US', {month:'short', day:'numeric'})}` };
}
...
function render() {
    ...
    if (activeTab === 'dashboard') {
        ...
        const periodBills = getSchedule().filter(r => r.date >= p.startStr && r.date <= p.endStr).reduce((s, r) => s + (r.actual ?? r.amount), 0);
        const periodSpent = state.spending.filter(s => s.date >= p.startStr && s.date <= p.endStr).reduce((s, x) => s + x.amount, 0);
        ...
        content.innerHTML = periodNav + `
            ...
            <div class="stat-grid" style="margin: 0 12px;">
                <div class="panel" style="margin:0; padding:10px;"><small>Bills</small><div style="font-weight:700; color:var(--danger)">${format(periodBills)}</div></div>
